    ...
    return new Promise((resolve, reject) => {
        const options = {
            ...
            headers: {
                ...
                'User-Agent': 'Power-Hour-Verification-Local'
            }
        };
...
async function verifyPowerHour() {
    console.log('⚡ Starting Power Hour Isolation Audit (LOCAL)...');
    ...
    try {
        ...
        const teamsRes = await request('GET', '/teams', null, token);
        const teams = teamsRes.data;
        if (!teams || teams.length === 0) throw new Error('No teams found to test with.');
        ...
        console.log('\n🌟 POWER HOUR LOGIC VERIFICATION: PASSED 🌟');
